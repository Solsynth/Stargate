...
func activateAccountFlow(c *gin.Context, d Deps, accountID uuid.UUID, activatedAt time.Time) error {
	if err := d.Store.AdminActivateAccount(c.Request.Context(), accountID, activatedAt); err != nil {
		return err
	}
...
func adminDeleteAccount(d Deps) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		now := time.Now().UTC()
		if err := d.Store.AdminSoftDeleteAccount(c.Request.Context(), accountID, now); err != nil {
			serverError(c, err, d)
			return
		}
		if d.Log != nil {
