...
// MarshalJSON normalizes nil audiences/scopes to [] (the C# always emits
// empty lists; the SDK tolerates both but parity is cheap).
func (s AuthSession) MarshalJSON() ([]byte, error) {
	type alias AuthSession
	normalized := s
	if normalized.Audiences == nil {
		normalized.Audiences = []string{}
	}
	if normalized.Scopes == nil {
		normalized.Scopes = []string{}
	}
...
// `as List<dynamic>` casts never see null.
func (c AuthChallenge) MarshalJSON() ([]byte, error) {
	type alias AuthChallenge
	normalized := c
	if normalized.BlacklistFactors == nil {
		normalized.BlacklistFactors = []string{}
	}
	if normalized.Audiences == nil {
		normalized.Audiences = []string{}
	}
...
// strict-cast by the Dart SDK (`as List<dynamic>`), so it must always
// serialize as [].
...
// MarshalJSON normalizes a nil Sessions list to [].
func (c AuthClientWithSessions) MarshalJSON() ([]byte, error) {
	type alias AuthClientWithSessions
	normalized := c
	if normalized.Sessions == nil {
		normalized.Sessions = []AuthSession{}
	}
	return json.Marshal((alias)(normalized))
}
